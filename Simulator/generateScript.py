...
def {element.id_bpmn}(env, name):
    ...
    with open('Simulator/files/results_{next(iter(elements))}.txt', 'a') as f:
        f.write(f'''
...
def checkServiceTasks(name):
    with open('Simulator/files/results_{next(iter(elements))}.txt', 'r') as f:
        content = f.readlines()"""
    for element in elements.values():
    ...
    """
    script += f"""
    with open('Simulator/files/results_{next(iter(elements))}.txt', 'w') as f:
